...
class NeurogymVanillaRNNNet(nn.Module):
    """Baseline: :class:`torch.nn.RNN` + optional :class:`~torch.nn.LayerNorm` + linear readout.

    Same I/O as :class:`NeurogymRNNNet`. When ``use_layer_norm`` is True, normalizes the RNN
    hidden dimension per timestep before the readout (``elementwise_affine=False``, same style
    as :class:`~inhibition.rnn.SimpleEERNN`).
    """
...
class NeurogymVanillaLSTMNet(nn.Module):
    """Same layout as :class:`NeurogymVanillaRNNNet` but with :class:`torch.nn.LSTM`.

    Same I/O as :class:`NeurogymRNNNet`. ``nonlinearity`` is accepted for API parity with
    the RNN variant but ignored (LSTM uses fixed gates).
    """
    ...
    def forward(self, x: torch.Tensor, return_layer_inputs: bool = False):
        ...
        logits = self.head(lstm_out.reshape(b * s, h)).reshape(b, s, self.n_actions)
        if return_layer_inputs:
            return logits, ()
        return logits
